...
train_dataset = TrainDataset(train, img_dir, mode='train', size=SIZE, transform=augmentation(mode='valid'))   # change mode
...
G = Generator()
D = Discriminator(norm='batch', pool_kernel_size=[4,4,2,2])
...
def main():
    ...
    for epoch in range(N_EPOCHS):
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                for real_rgb, _ in loaders_dict[phase]:
                    ...
                    ones = torch.ones(real_rgb.size()[0], 1, 4, 4).to(DEVICE)
                    zeros = torch.zeros(real_rgb.size()[0], 1, 4, 4).to(DEVICE)
                    ...
                    G_loss_adv = criterion_adv(d_out_fake, ones)
                    ...
                    D_loss_real = criterion_adv(d_out_real, ones)
                    D_loss_fake = criterion_adv(d_out_fake2, zeros)
                    D_loss = D_loss_real + D_loss_fake
                    G_optimizer.zero_grad();  D_optimizer.zero_grad()
                    D_loss.backward()
                    D_optimizer.step()
                    epoch_train_D_adv_loss += D_loss.item()
        ...
        plt.plot(df['train_D_adv_loss'], label='Discriminator Adv Loss', color='blue')
        plt.plot(df['train_G_adv_loss'], label='Generator Adv Loss', color='red')
